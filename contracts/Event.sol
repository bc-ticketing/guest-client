...
contract Event{
    ...
    function addFungibleTicketFactory(uint _ticketPrice, uint _numberTickets, string memory _metadataURI) public{
        ...
        fungibleTickets[fungibleTicketFactoryIndex] = EventLibrary.FungibleTicketFactory({
        ...
        fungibleTicketFactoryIndex++;
    }
    
    function buyFungibleTicket(uint _fungibleTicketFactoryId) public payable{
        // TODO Check if msg.sender is has verified ID in verification smart contract
        
        EventLibrary.FungibleTicketFactory storage ftf = fungibleTickets[_fungibleTicketFactoryId];
        require(msg.value == ftf.ticketPrice, "The value does not match the ticket price.");
        
        // if not all tickets have been issued, the buyer automatically buys from the event owner
        if(ftf.ticketIndex < ftf.numberTickets){
            issueFungibleTicket(ftf);
        }
        
        // if people want to sell tickets, the buyer automatically buys from the earliest seller
        else if(ftf.sellingQueueTail != ftf.sellingQueueHead){
            buyFromSellingQueue(ftf);
        }
    ...
    function issueFungibleTicket(EventLibrary.FungibleTicketFactory storage _ftf) internal {
        // issue the ticket
        _ftf.tickets[_ftf.ticketIndex] = EventLibrary.FungibleTicket({
            id: _ftf.ticketIndex,
            owner: msg.sender
        });
    ...
    function buyFromSellingQueue(EventLibrary.FungibleTicketFactory storage _ftf) internal{
        // transfer money
        (_ftf.sellingQueue[_ftf.sellingQueueHead].owner).transfer(_ftf.ticketPrice);
        
        // transfer ownership
        _ftf.tickets[_ftf.sellingQueue[_ftf.sellingQueueHead].id].owner = msg.sender;
        
        // remove user from the queue
        delete _ftf.sellingQueue[_ftf.sellingQueueHead];
        _ftf.sellingQueueHead++;
    }
    
    function joinBuyingQueue(EventLibrary.FungibleTicketFactory storage _ftf) internal{
        _ftf.buyingQueue[_ftf.buyingQueueTail] = msg.sender;
        _ftf.buyingQueueTail++;
    }
    
    function sellFungibleTicket(uint _fungibleTicketFactoryId, uint _ticketId) public{
        EventLibrary.FungibleTicketFactory storage ftf = fungibleTickets[_fungibleTicketFactoryId];
